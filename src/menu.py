...
class MenuPrincipal:
    def __init__(
        ...
        quantidade_faltas: QuantidadeDeFaltas,
        notas_boletim: NotasBoletim,
        utils: Utils,
    ):
    ...
    def __mostra_carregamento(self, message):
        ...
        # Mensagem de loading
        loading_label = Label(
            main_frame, text=message, font=("Arial", 16), bg="#f0f0f0", fg="#333333"
        )
        ...
        if tipo == "Atividades Pendentes":
            ...
            if not pdf:
                ...
                if resposta == "yes":
                    ...
                    self.notas_boletim.extrair_tabelas_pdf(
                        os.path.join(self.utils.caminho_projeto, pdf)
                    )
            ...
            if not pdf:
                ...
                if resposta == "yes":
                    ...
                    self.quantidade_faltas.extrair_tabelas_pdf(
                        os.path.join(self.utils.caminho_projeto, pdf)
                    )
    ...
    def __mostrar_consulta_especifica(self, tipo):
        ...
        info_label = Label(
            export_frame, text=info_text, font=("Arial", 10), bg="#f0f0f0", fg="#666666"
        )
        ...
        if dataframe_atual is not None and not dataframe_atual.empty:
            ...
            for index, row in dataframe_atual.iterrows():
                ...
                # Verifica se existe a coluna "Aprovacao" para aplicar a cor de fundo
                self.tree.tag_configure("branco", background="white")

                if "Aprovação" in colunas:
                    self.tree.tag_configure(tagname="vermelho", background="#ffe6e6")
                    self.tree.tag_configure(tagname="verde", background="#c8ffb9")

                    if valor == "Reprovado":
                        (
                            self.tree.insert(
                                "", "end", values=valores, tags=("vermelho",)
                            ),
                        )
                        continue
                    self.tree.insert("", "end", values=valores, tags=("verde",))
                else:
                    self.tree.insert("", "end", values=valores, tags=("branco",))
    ...
    def exportar_xlsx(self, tipo):
        ...
        if dataframe_atual is None or dataframe_atual.empty:
            messagebox.showwarning("Aviso", f"Não há dados de {tipo} para exportar!")
            return
    ...
    def exportar_csv(self, tipo):
        ...
        try:
            ...
            if filename:
                dataframe_atual.to_csv(filename, index=False, encoding="utf-8-sig")
                messagebox.showinfo("Sucesso", f"Arquivo salvo como:\n{filename}")
    ...
    def __obter_linha_selecionada(self):
        ...
        if not selected_items:
            messagebox.showwarning("Aviso", "Por favor, selecione uma linha na tabela.")
            return None
